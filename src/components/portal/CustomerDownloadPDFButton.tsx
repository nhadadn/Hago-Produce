...
export function CustomerDownloadPDFButton(props: Props) {
    ...
    try {
      const token =
        typeof window !== 'undefined'
          ? localStorage.getItem('customerAccessToken') || localStorage.getItem('accessToken')
          : null;
      ...
      if (!response.ok) {
        ...
        toast({ title: 'Error al generar PDF', description: message, variant: 'destructive' });
        return;
      }
  ...
  return (
    <Button {...buttonProps} onClick={handleClick} disabled={loading || disabled}>
      {loading ? (
        <>
